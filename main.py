...
print(f"=== V22 SANS MTRACE TOKEN={bool(TOKEN)} GROQ={bool(GROQ_KEY)} ===")
...
@flask_app.route('/')
def home(): return f"Bot {SIGNATURE} V22 LIVE"
...
def get_todays_fixtures():
    try:
        ...
        return m if m else ["Man City vs Arsenal"]
...
def predict_exact_score(match):
    ...
    try:
        comp=groq_client.chat.completions.create(model="openai/gpt-oss-20b",messages=[{"role":"user","content":f"Simule EA FC 26 en français, score exact + % + buteurs: {match}"}],temperature=0.4)
        return comp.choices[0].message.content
    except Exception as e: return f"🎯 {match} => 2-1 (60%) Erreur: {e}"
...
def get_host_info(host):
    ...
    try:
        ...
        try:
            ...
            if r.get("status")=="success": info.update({"country":f"{r.get('country')} - {r.get('city')}", "isp":r.get("isp"), "org":r.get("org")})
        except: pass
...
async def stats_cmd(update, context):
    save_user(update.effective_user.id)
    await update.message.reply_text(f"📊 STATS - {SIGNATURE} 👑\nTotal: {get_total_users()}\n\n{SIGNATURE}")
...
async def today_cmd(update,context):
    save_user(update.effective_user.id)
    await context.bot.send_chat_action(update.effective_chat.id,"typing")
    await update.message.reply_text("🔥 Matchs du jour...")
...
async def handle_photo(update, context):
    ...
    try:
        ...
        if groq_client:
            comp=groq_client.chat.completions.create(model="qwen/qwen3.6-27b",messages=[{"role":"user","content":[{"type":"text","text":f"{cap} /no_think Réponds français 4 lignes: marque modèle batterie processeur."},{"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}}]}],temperature=0.0,max_tokens=300)
        ...
        else: await update.message.reply_text("❌ GROQ_API_KEY manquant")
...
async def chat_gpt(update,context):
    ...
    try:
        ...
        comp=groq_client.chat.completions.create(model="openai/gpt-oss-20b",messages=[{"role":"system","content":f"Tu es {SIGNATURE}. Français + lingala, Roi Courageux, fun, serviable, tu réponds à tout."}]+conversations[uid][-10:],temperature=0.7)
        rep=comp.choices[0].message.content
    except Exception as e:
        print(f"CHAT FAIL {e}")
        if "bonjour" in low or "salut" in low: rep="Mbote Boss! 👑 Je suis COURAGEUX THE KING! Tape /start"
        elif "comment" in low: rep="Je vais bien Boss! Dis-moi: /exact, /today, /scan ou envoie photo!"
        else: rep=f"Yo Boss! Reçu: '{txt}'. Je suis là! Tape /start pour mes commandes 👑"
...
def main():
    print("Building V22 SANS MTRACE...")
